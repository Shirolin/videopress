...
```
* **如何使用**：在资源管理器中，选择你想要压缩的视频文件（支持多选），**右键 -> 发送到 -> 快速压缩视频** 即可自动开始压缩。压缩在后台静默进行，完成后弹出 Windows 系统通知提示压缩结果，无需等待命令行窗口。
...
* 🖥 **双入口无缝切换**：
  * **GUI 模式**：双击程序或不带参数运行时，展示基于 Wails + Svelte 的暖色压片台图形界面。支持文件拖拽、压缩队列管理、全局设置配置及 GPU 加速检测切换。
